...
#define _FTMEXT_METHOD_CHECK_INLET(m, e, i) do { \
  long _wi = (i); if(_wi != 0) { ftmext_error((e), "inlet %ld: doesn't understand message '%s'", (_wi), #m); goto _ftmext_exit_label; } \
} while(0)
...
FTS_API void ftmext_class_message(ftmext_class_t *cl, const char *name, fts_class_t *type, ftmext_method_t meth, const char *doc);
...
FTS_API void ftmext_class_inlet(ftmext_class_t *cl, int i, fts_class_t *type, const char *doc);
...
struct ftmext_outlet
{
  ...
  const char *doc; /**< outlet doc */
};

FTS_API void ftmext_class_outlet(ftmext_class_t *cl, int i, fts_class_t *type, const char *doc);
...
struct ftmext_attribute
{
  ...
  const char *doc; /**< attribute doc */
  struct ftmext_attribute *next;
};

FTS_API void ftmext_class_attribute(ftmext_class_t *cl, const char *name, fts_class_t *type, ftmext_method_t meth, int field, const char *doc);
