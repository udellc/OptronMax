...
#define safelogabsf(x) ((fabs(x) > LOG_ARG_MIN)? (logf(fabs(x))): (LOG_MIN))
#ifndef WIN32
float erfinvf (float x);
#endif

/* 8.68588963807 = 20 * log10(e), 0.11512925465 = 1 / (20 * log10(e)) */
...
FUN(cosh, coshf, "")
FUN(tanh, tanhf, "")
FUN(abs, fabs, ": get absolute value")
FUN(log, safelogf, "")
FUN(logabs, safelogabsf, ": logarithm of absolute value")
FUN(log10, log10f, 	"")
FUN(exp, expf,   	"")
#ifndef WIN32
FUN(erf, erff,   	": error function, i.e. cumulative gaussian")
FUN(erfc, erfcf,   	": complementary error function, i.e. 1 - cumulative gaussian")
FUN(erfinv, erfinvf,   	": inverse error function, i.e. inverse cumulative gaussian")
#endif
FUN(sqrt, sqrtf,  	"")
